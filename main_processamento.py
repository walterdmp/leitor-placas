...
class SistemaProcessamentoPlacas:
    ...
    def processar_imagem(self, caminho_imagem):
        ...
        if melhor_candidato:
            ...
            if dados:
                ...
                if status in ['BLOQUEADO', 'OCORRENCIA']:
                    ...
                    print(f"ALERTA: Veiculo {status} identificado: {placa_final}")
                else:
                    ...
                    print(f"REGISTRO: {placa_final} ({tipo}) - {proprietario}")
                    if melhor_candidato['corrigido']:
                        print(f"  (Leitura corrigida pelo banco de dados)")
            else:
                cor = AMARELO
                print(f"VISITANTE: {placa_final} nao consta na base.")
        ...
        cv2.imshow('Processamento', img_final)
        cv2.waitKey(0)

    def tenta_corrigir_pelo_banco(self, placa_lida):
        """Correção fuzzy de erros de OCR."""
    ...
    def executar_processamento(self):
        if not os.path.exists(self.pasta_imagens):
            print(f"ERRO: Pasta '{self.pasta_imagens}' nao encontrada.")
            return
        ...
        if alertas:
            print("\n---------------------------------------------")
            print("ALERTA: Permanencia Excedida!")
            for a in alertas:
